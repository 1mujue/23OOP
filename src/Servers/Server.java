...
public class Server {
            ...
            while (isStart) {
                ...
                try {
                    if (clientInformation != null) {
                        ...
                        this.sendObject(clientSocket, dynamic_client_informations);//send all online client list to client.
     ...
     * @author 赵龙淳
     * @version 1.0
     * @description: this class is very important,it supports the running of our server.
     ...
     * @author 赵龙淳
     * @version 1.0
     * @description: caution: only when a chat window is started can we get this thread start.
     * @date 2023/12/10 20:09
     */
    public class ClientThread implements Runnable {//caution: a client thread belongs only to one client.
        public ClientSocket clientSocket;
        private Object o;
        private ClientList clientList;

        public ClientThread(ClientSocket clientSocket) {//this is a thread only connected with server.
        ...
        private void sendMessageToOthers() throws SendMessageToOthersException {
            ...
            String text;
            int size = -1;
            Iterator<ClientThread> ITCSS = ClientThreads.iterator();//this is online client list with socket and Object Stream.
            ClientThread temp_CS;
            ArrayList<ClientInformation> chat_client_informations = null;
            if (clientList == null) {//we do not choose who we want to send message to,so we send the message globally.
                size = 0;
            ...
            if (size == -1) {//that means, the client list is not null.
                chat_client_informations = clientList.getClientInformations();
                if (chat_client_informations == null) {//although it is impossible to happen if everything works correctly.
                    size = 0;
                } else {
            ...
            while (ITCSS.hasNext()) {
                temp_CS = ITCSS.next();
                if (!temp_CS.clientSocket.getClientInformation().getName().equals(this.clientSocket.getClientInformation().getName())) {
                    if (tag.equals("Global:")) {
                        chat_client_informations = dynamic_client_informations.getClientInformations();//all online clients.
                        if (chat_client_informations == null) {//although it is impossible to happen if everything works correctly.
        ...
        private void addFriends() throws AddFriendsException {
            ...
            ArrayList<ClientInformation> add_clientInformations = clientList.getClientInformations();//these are who we want to add.
            ...
            for (ClientInformation clientInformation_in_add : add_clientInformations) {//who we want to add.
                for (ClientThread clientThread : ClientThreads) {
                    ...
                    if (clientInformation_in_client_thread.getName().equals(clientInformation_in_add.getName())) {
                        try {
                            ...
                            sendObject(clientThread.clientSocket, clientInformation_in_client_thread.getClient().getClientList("friend"));
                            addSuccess = true;
                            break;
        ...
        private void deleteFriends() throws DeleteFriendsException {
            ...
            ClientInformation owner_client_information = this.clientSocket.getClientInformation();//who wants to delete？
            ...
            for (ClientInformation client_information_in_delete : delete_clientInformations) {//delete the who from the list of whom he wants to delete.
                for (ClientThread clientThread : ClientThreads) {
                    ClientInformation clientInformation_in_client_thread = clientThread.clientSocket.getClientInformation();
                    if (clientInformation_in_client_thread.getName().equals(client_information_in_delete.getName())) {
                        try {
                            ClientList friend_client_list_of_deleter = getClientList(client_information_in_delete, "friend");
            ...
            try {
                while (isStart) {
                    ...
                    if (o instanceof ClientList) {
                        ...
                        } else if (clientList.getType().equals("add")) {//an add request from client.
